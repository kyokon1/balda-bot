...
function broadcastRooms(){
  ...
  rooms.forEach((room,id)=>{
    list.push({id,players:room.sockets.size,max:room.max,pass:!!room.pass});
  });
  io.emit('rooms',list);
}
...
io.on('connection',socket=>{
  ...
  socket.on('createRoom',data=>{
    ...
    rooms.set(id,{max:data.max,pass:data.pass||'',current:1,sockets:new Set()});
    broadcastRooms();
  });
  socket.on('joinRoom',data=>{
    ...
    socket.emit('joined',{room:data.room,player:playerNumber,current:room.current});
    broadcastRooms();
  });
...
bot.command('start',ctx=>{
  ctx.reply('Добро пожаловать в Балду!',{reply_markup:{keyboard:[[{text:'🎮 Играть в Балду',web_app:{url:'https://balda.store'}}]],resize_keyboard:true}});
});
